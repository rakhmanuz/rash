  ...
  return (
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="fixed inset-0 pointer-events-none">
        ...
        <div className="absolute inset-0 bg-[linear-gradient(to_right,rgba(51,65,85,0.06)_1px,transparent_1px),linear-gradient(to_bottom,rgba(51,65,85,0.06)_1px,transparent_1px)] bg-[size:3rem_3rem] [mask-image:radial-gradient(ellipse_80%_50%_at_50%_0%,#000_40%,transparent_100%)]" />
      </div>

      <header className="relative z-10 border-b border-slate-800/50 bg-slate-900/30 backdrop-blur-sm">
      ...
      <main className="relative z-10 max-w-lg mx-auto p-4 pb-8 space-y-6">
        {/* O'quvchi qidirish */}
        <section className="bg-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-5">
          ...
          <div className="flex gap-2">
            <div className="relative flex-1 group">
              ...
              <input
                ...
                className="w-full pl-12 pr-4 py-3 bg-slate-800/50 border border-slate-600/50 rounded-xl focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all placeholder-slate-500"
              />
            </div>
            <button
              type="button"
              ...
              disabled={loading}
              className="px-5 py-3 bg-emerald-600 hover:bg-emerald-500 rounded-xl font-medium disabled:opacity-50 transition-colors flex items-center gap-2 shrink-0"
            >
              {loading ? (
        ...
        {student && (
          <section
            className="bg-slate-900/50 backdrop-blur-sm border border-emerald-500/20 rounded-2xl p-5 animate-slide-up"
          >
            ...
            <div className="flex items-center gap-4">
              ...
              <div className="w-2 h-2 rounded-full bg-emerald-500 shrink-0" title="Topildi" />
            </div>
          </section>
        )}
        ...
        {success && (
          <div
            className="flex items-center gap-3 text-emerald-400 text-sm bg-emerald-500/10 border border-emerald-500/20 rounded-xl px-4 py-3"
            role="status"
          >
        ...
        <section className="bg-slate-900/50 backdrop-blur-sm border border-slate-700/50 rounded-2xl p-5 space-y-5">
          <div>
            <label className="flex items-center gap-2 text-sm font-medium text-slate-300 mb-3">
            ...
            <input
              ...
              placeholder="500 000"
              className="w-full px-4 py-4 bg-slate-800/50 border border-slate-600/50 rounded-xl text-lg font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500/50 focus:border-emerald-500/50 transition-all placeholder-slate-500"
            />
            ...
            <div className="flex flex-wrap gap-2 mt-3">
              {PRESET_AMOUNTS.map((val) => (
                <button
                  ...
                  className="px-3 py-1.5 text-sm rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-300 hover:text-white border border-slate-600/50 transition-colors"
                >
                  {formatAmount(val)}
                </button>
              ))}
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2 py-4 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-semibold rounded-xl shadow-lg shadow-emerald-500/20 hover:shadow-emerald-500/25 disabled:opacity-50 disabled:cursor-not-allowed disabled:hover:bg-emerald-600 transition-all duration-200 active:scale-[0.99]"
          >
            {loading ? (
              <>
                <Loader2 className="h-5 w-5 animate-spin" />
