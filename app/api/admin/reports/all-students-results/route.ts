...
export async function GET(request: NextRequest) {
  try {
    ...
    if (dates.length === 0) {
      return NextResponse.json({ error: 'Ma\'lumotlar topilmadi' }, { status: 404 })
    }
    ...
    return new NextResponse(excelBuffer, {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="barcha-oquvchilar-natijalari.xlsx"`,
      },
    })
  } catch (error) {
    console.error('Error generating all students results:', error)
