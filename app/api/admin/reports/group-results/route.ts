...
export async function GET(request: NextRequest) {
  try {
    ...
    const studentIds = group.enrollments.map((e: { studentId: string }) => e.studentId)
    ...
    allTestResults.forEach((tr: { studentId: string; correctAnswers: number; test: { date: Date; totalQuestions: number | null; type: string } }) => {
      ...
      const testData = { correct: tr.correctAnswers, total }
    ...
    // Data rows
    group.enrollments.forEach((enrollment: { student: { id: string; studentId: string | null; user: { name: string | null; username: string | null } } }, index: number) => {
