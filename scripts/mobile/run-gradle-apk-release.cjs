/**
 * Keystore + local.properties → cap sync → assembleRelease → mobile-app/output/rash-play-release.apk
 */
...
const src = path.join(android, 'app', 'build', 'outputs', 'apk', 'release', 'app-release.apk')
if (!fs.existsSync(src)) {
  console.error('APK topilmadi:', src)
  process.exit(1)
}
const outDir = path.join(root, 'mobile-app', 'output')
fs.mkdirSync(outDir, { recursive: true })
const dest = path.join(outDir, 'rash-play-release.apk')
fs.copyFileSync(src, dest)
console.log('')
console.log('=== TAYYOR APK ===')
console.log(dest)
